...
// Get correct image for product card
...
useEffect(() => {
  ...
  const delay = setTimeout(() => {
    ...
    // MATCH PRODUCTS (current category only)
...
}, [searchTerm, allProducts, categories]);useEffect(() => {
...
>
  ...
  {currentCategory?.name || "Collection"}
        ...
        {/* SEARCH SECTION */}
<div className="max-w-xl mx-auto mt-8 mb-14 relative">
  <input
    type="text"
...
{searchTerm.length >= 2 && (
      ...
      {noResult && (
        <div className="p-4 text-red-500 text-center">
          No product/category found with that name.
        </div>
      )}

{Array.isArray(searchResults) && searchResults.map((item) => {      const image =
  item.type === "category"
    ? item.image
