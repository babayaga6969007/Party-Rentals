...
const selectedDays =
 ...
 const handleProductQtyChange = (inc) => {
  ...
  // Vinyl wrap addon — size dropdown + image or hex
  ...
  const [vinylSizes, setVinylSizes] = useState([]);
  const [selectedVinylSizeKey, setSelectedVinylSizeKey] = useState("");
  const [vinylHex, setVinylHex] = useState("");
  const [vinylImageOrColor, setVinylImageOrColor] = useState("image");
...
const unitPrice = effectivePrice;
  ...
  if (isVinylSelected && vinylImageOrColor === "image" && vinylImageFile) {
    try {
  ...
  const addonsLine = Object.entries(selectedAddons).map(([optionId, a]) => ({
    ...
    vinylColor: optionId === vinylOptionId ? (vinylImageOrColor === "hex" ? "custom" : "") : "",
    vinylHex: optionId === vinylOptionId && vinylImageOrColor === "hex" && vinylHex.trim()
      ? (vinylHex.trim().startsWith("#") ? vinylHex.trim() : `#${vinylHex.trim()}`)
      : optionId === vinylOptionId ? "" : "",
    vinylImageUrl: optionId === vinylOptionId ? (vinylImageOrColor === "image" ? (resolvedVinylImageUrl || "") : "") : "",
    ...
    vinylSizeKey: optionId === vinylOptionId ? (selectedVinylSizeKey || "") : undefined,
    vinylSizeLabel: optionId === vinylOptionId ? (vinylSizes.find((s) => s.key === selectedVinylSizeKey)?.label || "") : undefined,
  }));
                      ...
                      {addon.optionId === vinylOptionId && selected && (
                        <div className="mt-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
                          ...
                          >
                            ...
                            {vinylSizes.map((s) => (
                              <option key={s._id || s.key} value={s.key}>{s.label}</option>
                            ))}
                          </select>
                          ...
                          <div className="flex gap-4 mb-3">
                            <label className="flex items-center gap-2 cursor-pointer">
                              <input
                                ...
                                onChange={() => { setVinylImageOrColor("image"); setVinylError(""); }}
                            ...
                            <>
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  if (file) {
                                    ...
                                    if (!ok) { setVinylError("Only JPG, JPEG, and PDF files are allowed."); e.target.value = ""; return; }
                                    if (file.size > 3 * 1024 * 1024) { toast.error("Image is too large. Maximum size is 3MB."); e.target.value = ""; return; }
                              ...
                              {vinylImagePreview && (
                                <div className="mt-3 flex items-center gap-3">
                                  ...
                                  <button type="button" onClick={() => { setVinylImageFile(null); setVinylImagePreview(""); setVinylImageUrl(""); }} className="text-sm text-red-600 hover:text-red-700">Remove image</button>
                                </div>
                              )}
                              {vinylImageFile && !vinylImagePreview && vinylImageFile.type === "application/pdf" && (
                                <div className="mt-3 flex items-center gap-3">
                                  ...
                                  <button type="button" onClick={() => { setVinylImageFile(null); setVinylImagePreview(""); setVinylImageUrl(""); }} className="text-sm text-red-600 hover:text-red-700">Remove</button>
                                </div>
                              )}
                          ...
                          {vinylImageOrColor === "hex" && (
                            <div className="flex items-center gap-3">
                              <input
                                type="text"
                                value={vinylHex}
                                onChange={(e) => { setVinylHex(e.target.value); setVinylError(""); }}
                              ...
                              {vinylHex.trim() && (
                                <div
                                  ...
                                  style={{ backgroundColor: /^#?[0-9A-Fa-f]{3,6}$/.test(vinylHex.trim()) ? (vinylHex.trim().startsWith("#") ? vinylHex.trim() : `#${vinylHex.trim()}`) : "transparent" }}
                                  title="Preview"
                                />
                              )}
